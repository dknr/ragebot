# Builds ragebot (a single Go binary) plus the embedded sentiment model and ONNX
# Runtime shared library. The model blobs are regenerated from HuggingFace, so
# a clean checkout needs Go (1.21+ with cgo), Rust + cargo (to build
# libtokenizers.a from github.com/daulet/tokenizers), and Python 3 (to download
# + convert the model and fetch ORT).
...
.PHONY: all build run model ort vet clean

all: build

build: $(BLOBS)/model.onnx.zst $(BLOBS)/tokenizer.json.zst $(BLOBS)/ort.so.zst libtokenizers.a
	CGO_LDFLAGS="-L$(CURDIR)" go build -tags goolm -o $(BIN) .

run: build
	./$(BIN) -config config.json

## Download + convert the model: fp32 export, int8 quantization, tokenizer.json.
